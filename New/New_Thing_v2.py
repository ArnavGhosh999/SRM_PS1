...
def local_outlier_factor_detection(features, contamination=0.05):
    ...
    anomaly_binary = (predictions == -1).astype(int)
    print(f"  Detected {anomaly_binary.sum()} anomalies ({anomaly_binary.sum()/len(features)*100:.1f}%)")
    return anomaly_binary, lof.negative_outlier_factor_

def one_class_svm_detection(features, nu=0.05):
    ...
    anomaly_binary = (predictions == -1).astype(int)
    print(f"  Detected {anomaly_binary.sum()} anomalies ({anomaly_binary.sum()/len(features)*100:.1f}%)")
    return anomaly_binary, oc_svm.decision_function(features_scaled)

def autoencoder_anomaly_detection(features, threshold=None, epochs=20, batch_size=32):
    ...
    autoencoder.fit(features_scaled, features_scaled, epochs=epochs, batch_size=batch_size, verbose=0)

    reconstructions = autoencoder.predict(features_scaled)
    mse = np.mean(np.square(features_scaled - reconstructions), axis=1)
    if threshold is None:
        threshold = np.percentile(mse, 95)
    anomaly_binary = (mse > threshold).astype(int)
    print(f"  Detected {anomaly_binary.sum()} anomalies ({anomaly_binary.sum()/len(features)*100:.1f}%)")
    return anomaly_binary, mse
...
def analyze_signals_with_sliding_window(df, window_size=100, step_size=25, contamination=0.08):
    ...
    for pair_name, voltage_data, current_data in pairs:
        ...
        lof_anomalies, lof_scores = local_outlier_factor_detection(windows, contamination)
        svm_anomalies, svm_scores = one_class_svm_detection(windows, contamination)
        ae_anomalies, ae_scores = autoencoder_anomaly_detection(windows, contamination)
        ...
        results = {
            ...
            'lof_anomalies': int(np.sum(lof_anomalies)),
            'svm_anomalies': int(np.sum(svm_anomalies)),
            'ae_anomalies': int(np.sum(ae_anomalies)),
            ...
            'lof_percentage': float(np.sum(lof_anomalies) / len(windows) * 100),
            'svm_percentage': float(np.sum(svm_anomalies) / len(windows) * 100),
            'ae_percentage': float(np.sum(ae_anomalies) / len(windows) * 100),
            ...
            'lof_anomaly_windows': window_indices[lof_anomalies == 1].tolist(),
            'svm_anomaly_windows': window_indices[svm_anomalies == 1].tolist(),
            'ae_anomaly_windows': window_indices[ae_anomalies == 1].tolist(),
        ...
        print(f"    LOF: {results['lof_anomalies']} ({results['lof_percentage']:.1f}%)")
        print(f"    SVM: {results['svm_anomalies']} ({results['svm_percentage']:.1f}%)")
        print(f"    Autoencoder: {results['ae_anomalies']} ({results['ae_percentage']:.1f}%)")
        ...
        plot_enhanced_anomaly_results(
            ...
            lof_anomalies, svm_anomalies, ae_anomalies,
            pair_name
        )
...
def plot_enhanced_anomaly_results(voltage_data, current_data, window_indices,
    ...
    lof_anomaly_indices = window_indices[lof_anomalies == 1]
    svm_anomaly_indices = window_indices[svm_anomalies == 1]
    ae_anomaly_indices = window_indices[ae_anomalies == 1]
    ...
    # Plot 5: Voltage with LOF anomalies
    axes[2, 0].plot(voltage_data, color='blue', alpha=0.7, linewidth=0.8, label='Voltage')
    if len(lof_anomaly_indices) > 0:
        axes[2, 0].scatter(lof_anomaly_indices, voltage_data[lof_anomaly_indices], 
                          color='purple', s=15, alpha=0.8, label=f'LOF Anomalies ({len(lof_anomaly_indices)})')
    axes[2, 0].set_title(f'Voltage Signal with LOF Anomalies', fontsize=12, fontweight='bold')
    axes[2, 0].set_xlabel('Sample Index', fontsize=11)
    axes[2, 0].set_ylabel(f'Voltage (Pair {pair_name})', fontsize=11)
    axes[2, 0].legend()
    axes[2, 0].grid(True, alpha=0.3)
    
    # Plot 6: Current with LOF anomalies
    axes[2, 1].plot(current_data, color='red', alpha=0.7, linewidth=0.8, label='Current')
    if len(lof_anomaly_indices) > 0:
        axes[2, 1].scatter(lof_anomaly_indices, current_data[lof_anomaly_indices], 
                          color='darkviolet', s=15, alpha=0.8, label=f'LOF Anomalies ({len(lof_anomaly_indices)})')
    axes[2, 1].set_title(f'Current Signal with LOF Anomalies', fontsize=12, fontweight='bold')
    axes[2, 1].set_xlabel('Sample Index', fontsize=11)
    axes[2, 1].set_ylabel(f'Current (Pair {pair_name})', fontsize=11)
    axes[2, 1].legend()
    axes[2, 1].grid(True, alpha=0.3)
    
    # Plot 7: Voltage with SVM anomalies
    axes[3, 0].plot(voltage_data, color='blue', alpha=0.7, linewidth=0.8, label='Voltage')
    if len(svm_anomaly_indices) > 0:
        axes[3, 0].scatter(svm_anomaly_indices, voltage_data[svm_anomaly_indices], 
                          color='cyan', s=15, alpha=0.8, label=f'SVM Anomalies ({len(svm_anomaly_indices)})')
    axes[3, 0].set_title(f'Voltage Signal with SVM Anomalies', fontsize=12, fontweight='bold')
    axes[3, 0].set_xlabel('Sample Index', fontsize=11)
    axes[3, 0].set_ylabel(f'Voltage (Pair {pair_name})', fontsize=11)
    axes[3, 0].legend()
    axes[3, 0].grid(True, alpha=0.3)
    
    # Plot 8: Current with SVM anomalies
    axes[3, 1].plot(current_data, color='red', alpha=0.7, linewidth=0.8, label='Current')
    if len(svm_anomaly_indices) > 0:
        axes[3, 1].scatter(svm_anomaly_indices, current_data[svm_anomaly_indices], 
                          color='darkcyan', s=15, alpha=0.8, label=f'SVM Anomalies ({len(svm_anomaly_indices)})')
    axes[3, 1].set_title(f'Current Signal with SVM Anomalies', fontsize=12, fontweight='bold')
    axes[3, 1].set_xlabel('Sample Index', fontsize=11)
    axes[3, 1].set_ylabel(f'Current (Pair {pair_name})', fontsize=11)
    axes[3, 1].legend()
    axes[3, 1].grid(True, alpha=0.3)
    
    # Plot 9: Autoencoder scores
    axes[4, 0].plot(window_indices, ae_scores, color='green', alpha=0.7, linewidth=1, label='AE Scores')
    if len(ae_anomaly_indices) > 0:
        axes[4, 0].scatter(ae_anomaly_indices, ae_scores[ae_anomalies == 1], 
                          color='lime', s=15, alpha=0.8, label='AE Anomalies')
    axes[4, 0].set_title(f'Autoencoder Anomaly Scores', fontsize=12, fontweight='bold')
    axes[4, 0].set_xlabel('Window Center Index', fontsize=11)
    axes[4, 0].set_ylabel('Anomaly Score', fontsize=11)
    axes[4, 0].legend()
    axes[4, 0].grid(True, alpha=0.3)
    
    # Plot 10: All anomaly scores together
    axes[4, 1].plot(window_indices, iso_scores, color='blue', alpha=0.7, linewidth=1, label='IF Scores')
    axes[4, 1].plot(window_indices, xgb_scores, color='orange', alpha=0.7, linewidth=1, label='XGB Scores')
    axes[4, 1].plot(window_indices, lof_scores, color='purple', alpha=0.7, linewidth=1, label='LOF Scores')
    axes[4, 1].plot(window_indices, svm_scores, color='cyan', alpha=0.7, linewidth=1, label='SVM Scores')
    axes[4, 1].axhline(y=0.5, color='black', linestyle='--', alpha=0.5, label='Threshold')
    axes[4, 1].set_title(f'Anomaly Scores Comparison', fontsize=12, fontweight='bold')
    axes[4, 1].set_xlabel('Window Center Index', fontsize=11)
    axes[4, 1].set_ylabel('Anomaly Score / Probability', fontsize=11)
    axes[4, 1].legend()
...
def create_comprehensive_summary(all_results):
    ...
    lof_counts = [all_results[pair]['lof_anomalies'] for pair in pairs]
    svm_counts = [all_results[pair]['svm_anomalies'] for pair in pairs]
    ae_counts = [all_results[pair]['ae_anomalies'] for pair in pairs]
    ...
    lof_percentages = [all_results[pair]['lof_percentage'] for pair in pairs]
    svm_percentages = [all_results[pair]['svm_percentage'] for pair in pairs]
    ae_percentages = [all_results[pair]['ae_percentage'] for pair in pairs]
    
    # Plot 1: Anomaly counts comparison
    x = np.arange(len(pairs))
    width = 0.15
    
    axes[0, 0].bar(x - 2*width, iso_counts, width, label='Isolation Forest', color='blue', alpha=0.7)
    axes[0, 0].bar(x - width, xgb_counts, width, label='XGBoost', color='orange', alpha=0.7)
    axes[0, 0].bar(x, lof_counts, width, label='LOF', color='purple', alpha=0.7)
    axes[0, 0].bar(x + width, svm_counts, width, label='SVM', color='cyan', alpha=0.7)
    axes[0, 0].bar(x + 2*width, ae_counts, width, label='Autoencoder', color='green', alpha=0.7)
    axes[0, 0].bar(x + 3*width, overlap_counts, width, label='Overlap', color='red', alpha=0.7)
    ...
    axes[0, 1].bar(x + 3*width/2, lof_percentages, width, label='LOF', color='purple', alpha=0.7)
    axes[0, 1].bar(x + 5*width/2, svm_percentages, width, label='SVM', color='cyan', alpha=0.7)
    axes[0, 1].bar(x + 7*width/2, ae_percentages, width, label='Autoencoder', color='green', alpha=0.7)
...
def save_enhanced_results(df_formatted, stats, all_results):
    ...
    for pair_name, results in all_results.items():
        summary_data.append({
            ...
            'lof_anomalies': results['lof_anomalies'],
            'svm_anomalies': results['svm_anomalies'],
            'ae_anomalies': results['ae_anomalies'],
            'overlap_anomalies': results['overlap_anomalies'],
            'iso_percentage': results['iso_percentage'],
            'xgb_percentage': results['xgb_percentage'],
            'lof_percentage': results['lof_percentage'],
            'svm_percentage': results['svm_percentage'],
            'ae_percentage': results['ae_percentage'],
...
def main():
    ...
    for pair_name, results in all_results.items():
        ...
        print(f"  - LOF: {results['lof_anomalies']} anomalies ({results['lof_percentage']:.1f}%)")
        print(f"  - SVM: {results['svm_anomalies']} anomalies ({results['svm_percentage']:.1f}%)")
        print(f"  - Autoencoder: {results['ae_anomalies']} anomalies ({results['ae_percentage']:.1f}%)")
